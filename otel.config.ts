...
/** No-op without a key. @vercel/otel honors incoming W3C `traceparent` headers by default, so a
 *  span started by a sending app (bam-landing-page, shop-witus, …) continues here as the same
 *  distributed trace. The ingest route additionally persists that header with the submission so
 *  the ASYNC hop to the triage agent can continue the trace later (see app/api/ingest/route.ts). */
